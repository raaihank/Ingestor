...
def _parse_hf_spec(spec: str) -> tuple[str, str | None, str | None]:
    # Supports formats like: "owner/name", "owner/name:config", "owner/name@rev", "owner/name:config@rev"
    revision = None
    path = spec
    name = None
    if "@" in path:
...
def _iter_rows(ds, dataset_name: str, license_id: str | None):
    for idx, row in enumerate(ds):
        text = row.get("text") or row.get("prompt") or row.get("content") or ""
        label = row.get("label")
        meta = {k: v for k, v in row.items() if k not in {"text", "prompt", "label", "content"}}
...
def iter_huggingface(dataset_name: str) -> Generator[Dict, None, None]:
    ...
    try:
        ds = load_dataset(path, name=name, split="train", token=token, revision=revision)
        info = getattr(ds, "info", None)
        if info is not None:
            lic = getattr(info, "license", None)
            if lic is not None:
                license_id = str(lic)
        yield from _iter_rows(ds, dataset_name, license_id)
        return
    except Exception:
        pass
    ...
    for split_name in getattr(ds_dict, "keys", lambda: [])():
        ...
        yield from _iter_rows(split_ds, f"{dataset_name}:{split_name}", license_id)
